...
public class GearStatsTool : Singleton<GearStatsTool>
{
    ...
    {
        PlayerPrefs.DeleteKey(gearType + DATAKEY);
    }

    public void OnClickUnlockAllGears()
    {
        GearType type = gearsStatsManager.GetCurrentGearType();
        GearStatsList gearStatsList = gearsStatsManager.GetGearStatsDic()[type];
        GearConfig gearConfig = Array.Find(gearsStatsManager.GetGearConfigs(), config => config.type == type);
        SObjGearsStatsConfig[] configs = gearConfig.gearsStatsConfigs;
        ...
        PlayerPrefs.SetString(type + DATAKEY, JsonUtility.ToJson(gearStatsList));
        gearsStatsManager.SetTotalOwnedEffectValue(gearType);
        gearsStatsManager.SetGearItems(type);
    }

    public void SetPoints(GearItem gearItem, GearStats gearStats, SObjGearsStatsConfig gearsStatsConfig, int point)
    {
        gearStats.numberOfPoints += point;
        GearStatsList gearStatsList = gearsStatsManager.GetGearStatsDic()[gearsStatsConfig.type];
        PlayerPrefs.SetString(gearsStatsConfig.type + DATAKEY, JsonUtility.ToJson(gearStatsList));
        gearsStatsManager.SetGearInfoUI(gearStats, gearsStatsConfig.gearSpt, gearItem, gearsStatsConfig);
    }
}
